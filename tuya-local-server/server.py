...
class DeviceHandler:
    def __init__(self, characteristics_data):
        self.characteristics_data = characteristics_data      
        self.device_id = characteristics_data['id']
        self.ip = characteristics_data['ip']
        self.key = characteristics_data['key']
        self.version = float(characteristics_data['version'])
        self.mappings = self.create_mappings()

    def create_mappings(self):
        mappings = {}
        if 'mapping' in self.characteristics_data:
            mapping = self.characteristics_data['mapping']
            for dpsKey, info in mapping.items():
                if 'code' in info:
                    dps_code = info['code']
                    dps_type = info['type']
                    dps_values = info['values']
                    mappings[dps_code] = {
                        'dpsKey': int(dpsKey),
                        'type': dps_type,
                        'values': dps_values
    ...
    def get_dps_key_from_code(self, dps_code):
        return self.mappings[dps_code]['dpsKey']
        for dpsKey, info in self.mappings.items():
            log.debug(info)
            if str(dps_code) == str(dpsKey):
                return int(info['dpsKey'])
        return None

    def get_dps_value_from_code(self, dps_code, value):
        dps_type = self.mappings[dps_code]['type']
        if dps_type == 'Boolean':
            value_lower = str(value).strip().lower()
            if value_lower == 'true' or value_lower == '1':
                return True
            elif value_lower == 'false' or value_lower == '0':
                return False
        elif dps_type == 'Integer':
            return int(value)
        elif dps_type == 'Enum':
            return str(value)
        elif dps_type == 'Bitmap':
            return int(value, 2)
        return value
...
def tuyaLoadJson():
    ...
    # Create a dictionary with keys as IDs or names of each device in tdevices
    tuyadevicesdict = {}
    for device in tdevices:
        if 'id' in device:
            tuyadevicesdict[device['id']] = device
        if 'name' in device:
            tuyadevicesdict[device['name']] = device

    return tdevices, tuyadevicesdict
...
def tuyaLoadConfig():
    ...
    return config
global tuyadevicesdict
tuyadevices, tuyadevicesdict = tuyaLoadJson()
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path.startswith('/setmultiple/'):
            try:
                # ['', 'set', 'deviceid']
                (ignore1, ignore2, id) = self.path.split('/')
                ...
                if(id in deviceslist):
                    ...
                    device_handler = DeviceHandler(tuyadevicesdict[id])
                    ...
                    for key, value in payload.items():
                        dps_key = device_handler.get_dps_key_from_code(key)
                        if dps_key is not None:
                            dps_value = device_handler.get_dps_value_from_code(key, value)
                            converted_data[dps_key] = dps_value
                    message = formatreturn(d.set_multiple_values(converted_data,nowait=True))
                    d.close()
                else:
